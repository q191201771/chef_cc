...
void trans_endian(char *data, int len)
{
    union {
        char c;
        int i;
...
int decode_chunk_head(const char *raw_head, chunk_head *ch)
{
    ...
    trans_endian((char *)&ch->id_, sizeof ch->id_);
    memcpy(&ch->type_, raw_head + 8, sizeof ch->type_);
    trans_endian((char *)&ch->type_, sizeof ch->type_);
    memcpy(&ch->reserved_, raw_head + 16, sizeof ch->reserved_);
    trans_endian((char *)&ch->reserved_, sizeof ch->reserved_);
    memcpy(&ch->body_len_, raw_head + 20, sizeof ch->body_len_);
    trans_endian((char *)&ch->body_len_, sizeof ch->body_len_);
